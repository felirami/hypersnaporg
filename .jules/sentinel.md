...
**Learning:** The native `fetch` API in Node.js does not have a default timeout, leaving the application vulnerable to server resource exhaustion if an external service stops responding.
**Prevention:** Always use `signal: AbortSignal.timeout(TIMEOUT_MS)` when calling external APIs using `fetch`.
